...
from fleetrl.fleet_env import env_util
...
from fleetrl_2.jobs.environment_creation_job import EpisodeParams, MiscParams, NormalizationStrategy
...
class FleetEnv(gym.Env):
    ...
    def __init__(self,
                 environment_dataset_job: EnvironmentDatasetJob,
        ...
        self.model_frequency = str(model_step_size) + model_time_unit.value
        self.model_time_unit = model_time_unit
        self.model_step_size = model_step_size
        ...
        self.site_parameters = environment_dataset_job.site_parameters_job
        self.schedule_generation_job = environment_dataset_job.schedule_generation_job
        ...
        # Loading configs
        self.score_config = reward_function_job
        self.ev_config = environment_dataset_job.ev_config_job

        # redundant
        # setting seed
        self.seed = environment_dataset_job.seed
        ...
        self.include_price = environment_dataset_job.site_parameters_job.site_data.external_electricity_price is not None
        self.include_building_load = environment_dataset_job.auxiliary_data._building_load is not None
        self.include_pv = environment_dataset_job.auxiliary_data._pv_production is not None
        self.aux_flag = auxiliary_data  # include auxiliary information

        # conduct normalization of observations
        if normalization_strategy == NormalizationStrategy.NONE:
            self.normalize_in_env = False

        if normalization_strategy == NormalizationStrategy.ORACLE:
            self.normalize_in_env = True

        # Setting paths and file names
        # path for input files, needs to be the same for all inputs
        self.path_name = environment_dataset_job._dir_root

        # EV schedule database
        # generating own schedules or importing them
        self.schedule_name = environment_dataset_job.schedule_generation_job.schedule_output_name

        # Price databases
        self.spot_name = environment_dataset_job.site_parameters_job.site_data.external_electricity_price
        self.tariff_name = environment_dataset_job.site_parameters_job.site_data.external_electricity_tariff

        # Building load database
        self.building_name = environment_dataset_job.auxiliary_data._building_load

        # PV database is the same in this case
        if environment_dataset_job.auxiliary_data._pv_production is not None:
            self.pv_name = environment_dataset_job.auxiliary_data._pv_production
        else:
            self.pv_name = self.building_name

        # TODO: check where use-case is used
        ...
        self.calc_deg = misc_params.battery_degradation_type
        ...
        # initial degree of battery degradation, assumed equal for all cars
        self.initial_soh = self.ev_config.battery.initial_state_of_health
        # How much excess time the car should at least have to charge
        self.min_laxity: float = self.ev_config.min_laxity
        ...
        self.data_logger: DataLogger = DataLogger(self.episode_params.episode_length * self.time_steps_per_hour)

        self.real_time = self.misc_params.real_time

        # Loading the inputs
        self.data_loader: DataLoader = DataLoader(path_name=self.path_name,
        ...
        self.target_soc: np.ndarray = np.ones(self.num_cars) * self.ev_config.battery.target_soc

        if self.include_building_load:
            max_load = max(self.db["load"])
        else:
            max_load = 0  # building load not considered in that case
            print('Max load set to zero because building load not considered...')
        ...
        low_obs, high_obs = env_util.detect_dim_and_bounds(include_price=self.include_price,
                                                           num_cars=self.num_cars,
    ...
    def step(self, actions: np.array) -> tuple[np.array, float, bool, bool, dict]:
        ...
        while True:
            ...
            self.episode.time_conf.time_steps_per_hour = int(1 / np.copy(self.episode.dt))
            ...
            there = self.db["There"][
                self.db["date"] == self.episode.time].values  # plugged in y/n (before next time step)

            # parse the action to the charging function and receive the soc, next soc, reward and cashflow
            (self.episode.soc,
             self.episode.next_soc,
             reward, cashflow,
             self.charge_log,
             self.episode.events) = self.ev_charger.charge(db=self.db,
                                                           num_cars=self.num_cars,
                                                           actions=actions,
            ...
            if overloaded_flag:
                ...
                overload_penalty = self.score_config.penalty.overloading_penalty(relative_loading)
                    ...
                    if self.company == CompanyType.Caretaker:
                        ...
                        if (self.episode.time.hour > 11) and (self.episode.time.hour < 15):
                            ...
                            if self.ev_config.target_soc_lunch - self.episode.soc[car] > self.eps:
                                ...
                                current_soc_pen = self.score_config.penalty.soc_violation_penalty(soc_missing)
                                ...
                                reward += self.score_config.reward.fully_charged_reward  # reward for fully charging the car
    ...
    def change_markups(self):
        if self.site_parameters.fixed_markup is not None:
            self.ev_config.fixed_markup = self.site_parameters.fixed_markup
        if self.site_parameters.variable_multiplier is not None:
            self.ev_config.variable_multiplier = self.site_parameters.variable_multiplier
        if self.site_parameters.feed_in_deduction is not None:
            self.ev_config.feed_in_deduction = self.site_parameters.feed_in_deduction

    def adjust_score_config(self):
        if self.score_config.ignore.price:
            self.score_config.price_multiplier = 0
            print('Note: Money spent on electricity does not play a role for rewards anymore!')
        if self.score_config.ignore.overloading:
            self.score_config.penalty_overloading = 0
            print('Note: Overloading the grid connection will not trigger penalties anymore!')
        if self.score_config.ignore.invalid_action:
            self.score_config.penalty_invalid_action = 0
        if self.score_config.ignore.overcharging:
